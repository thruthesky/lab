...
void main() {
  group('generateFirewalldBlockCommands Tests', () {
...
''';
      ...
      List<String> actualCommands = generateFirewalldBlockCommands(ips);
...
''';
  ...
  group('generateUfwBlockCommands Tests', () {
    test('Example from problem description', () {
      String ips = '''
1.2.3.4
1.2.3.5
...
''';
      List<String> expectedCommands = [
        "ufw deny from 2.3.0.0/16",
        "ufw deny from 3.4.0.0/16",
        "ufw deny from 1.2.3.0/24",
        "ufw deny from 8.8.8.8",
      ];
      List<String> actualCommands = generateUfwBlockCommands(ips);
      actualCommands.sort(); // Sort for consistent comparison
      expectedCommands.sort();
      expect(actualCommands, equals(expectedCommands));
    });
...
''';
      List<String> expectedCommands = [
        "ufw deny from 2.2.0.0/16",
        "ufw deny from 4.4.0.0/16",
        "ufw deny from 5.5.0.0/16",
        "ufw deny from 1.2.3.0/24",
        "ufw deny from 100.200.0.0/24",
        "ufw deny from 3.3.3.0/24",
        "ufw deny from 6.6.6.0/24",
        "ufw deny from 1.9.9.9",
        "ufw deny from 100.201.0.125",
      ];
      List<String> actualCommands = generateUfwBlockCommands(ips);
      expect(actualCommands, equals(expectedCommands));
    });
