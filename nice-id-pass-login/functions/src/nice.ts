...
export class NiceApi {
   ...
   * @returns 대칭키
   */
  static generateSymmetricKey(token_val: string): SymmetricKey {
    const password = `${this.dt}${this.dt}${token_val.trim()}`;
   ...
   */
  static decryptRequestData(encrypted: string, symmetricKey: SymmetricKey) {
    const algorithm = "aes-128-cbc";
    const key = symmetricKey.key;
    const iv = symmetricKey.iv;
   ...
   */
  static hmac256(encrypted: string, hmac_key: string): string {
